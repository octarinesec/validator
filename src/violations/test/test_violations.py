...
def test_run_return_instances(new_file_violations_instance, mocker):
    ...
    mocker.patch("violations.violations.octactl", return_value=load_test_data('single_resource_change'))
    fv_run = new_file_violations_instance._run()
    assert isinstance(new_file_violations_instance, Violations)
    assert isinstance(fv_run, Violations)
...
def test_run_empty_octactl_results(new_file_violations_instance, mocker):
    ...
    mocker.patch("violations.violations.octactl",  return_value=load_test_data('no_resource_change'))
    fv_run = new_file_violations_instance._run()
    assert isinstance(fv_run, Violations)
